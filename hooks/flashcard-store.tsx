...
export const [FlashcardProvider, useFlashcards] = createContextHook(() => {
  ...
  const updateCardAfterReview = useCallback(
    async (cardId: string, difficulty: "again" | "hard" | "good" | "easy") => {
      ...
      newStats.achievements = newStats.achievements.map((achievement) => {
        ...
        if (achievement.id === "cards_100") {
          return {
            ...achievement,
            progress: Math.min(newStats.totalCardsStudied, 100),
          };
        }
        return achievement;
      });
  ...
  const getDueCards = useCallback(
    (deckId: string): Flashcard[] => {
      const now = Date.now();
      return cards.filter(
        (card) => card.deckId === deckId && card.nextReview <= now
      );
    },
    [cards]
  );
